...
def load_data():
	...
	labels_train = []
	docs_train = []
	with open('us_train.text', 'r') as f:
		for line in f:
	...
	with open('us_test.text', 'r') as f:
		for line in f:
			docs_test.append(' '.join(list(line)))
	category_num = 20
	...
	vocab_size = len(Counter(' '.join(docs)))
...
checkpoint = ModelCheckpoint('EP-W.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
